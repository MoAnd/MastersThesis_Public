...
# ggsave("WeibullProcess.png", device = "png", path = "Plots/ModelDynamics", width = 8, height = 4)


plt2 <- df %>% mutate(Home_int = alph_h * bet_h * Time^(bet_h - 1)) %>% mutate(Away_int = alph_a * bet_a * Time^(bet_a - 1)) %>% 
...
# ggsave("WeibullProcess_Intensity.png", device = "png", path = "Plots/ModelDynamics", width = 8, height = 4)
